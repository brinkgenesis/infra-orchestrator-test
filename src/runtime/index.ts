...
export class CircuitBreaker {
  ...
  constructor(opts: CircuitBreakerOptions) {
    if (opts.failureThreshold < 1 || !Number.isFinite(opts.failureThreshold) || !Number.isInteger(opts.failureThreshold)) {
...
export class Bulkhead {
  ...
  constructor(maxConcurrent: number, maxQueue = Infinity) {
    if (maxConcurrent < 1 || !Number.isFinite(maxConcurrent) || !Number.isInteger(maxConcurrent)) {
      throw new Error('maxConcurrent must be a positive integer');
    }
    if (maxQueue < 0 || (!Number.isInteger(maxQueue) && maxQueue !== Infinity)) {
      throw new Error('maxQueue must be a non-negative integer or Infinity');
    }
