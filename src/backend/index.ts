...
/** Creates an in-memory response cache with TTL and max-entry eviction. */
export function createResponseCache(config: CacheConfig): ResponseCacheFn {
...
export interface RequestTracker {
  start(id: string, timeoutMs?: number): TrackedRequest;
  finish(id: string): number | undefined;
  isTimedOut(id: string, now?: number): boolean;
  active(): ReadonlyArray<TrackedRequest>;
  activeCount(): number;
  sweepTimedOut(now?: number): string[];
}

/** Creates a request tracker that monitors in-flight requests and detects timeouts. */
export function createRequestTracker(config: RequestTrackerConfig): RequestTracker {
  ...
  return {
    ...
    finish(id: string): number | undefined {
      ...
      inflight.delete(id);
      return Date.now() - req.startedAt;
    },

    isTimedOut(id: string, now = Date.now()): boolean {
      const req = inflight.get(id);
      if (!req) return false;
      return now - req.startedAt >= req.timeoutMs;
    },
    ...
    sweepTimedOut(now = Date.now()): string[] {
      const swept: string[] = [];
...
/** Extracts named path parameters from a URL path using a pattern with :param placeholders. */
...
/** Generates a cache key from method and path, suitable for use with createResponseCache. */
...
export interface RequestBatcher<T> {
  add(item: T): number;
  flush(): T[];
  pending(): number;
...
/** Creates a request batcher that accumulates items and flushes them when the batch is full or on demand. */
export function createRequestBatcher<T>(config: RequestBatcherConfig): RequestBatcher<T> {
  let batch: T[] = [];
...
/** Returns true if the batcher has reached its configured max batch size. */
export function isBatchReady<T>(batcher: RequestBatcher<T>, config: RequestBatcherConfig): boolean {
  return batcher.pending() >= config.maxBatchSize;
}
...
export interface CircuitBreaker {
  ...
  getStatus(now?: number): CircuitBreakerStatus;
  recordSuccess(): void;
  recordFailure(now?: number): void;
  reset(): void;
  isCallPermitted(now?: number): boolean;
}

/** Creates a circuit breaker that tracks failures and opens when the threshold is exceeded. */
export function createCircuitBreaker(config: CircuitBreakerConfig): CircuitBreaker {
  ...
  function resolveState(now: number): CircuitState {
  ...
  return {
    getState(now = Date.now()): CircuitState {
      return resolveState(now);
    },

    getStatus(now = Date.now()): CircuitBreakerStatus {
      return {
        state: resolveState(now),
        failures,
        successes,
        lastFailureTime,
      };
    ...
    isCallPermitted(now = Date.now()): boolean {
      const current = resolveState(now);
      return current !== 'open';
    },
  };
}
